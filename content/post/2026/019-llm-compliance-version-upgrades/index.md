...
3.  **Explicit Minimum Bounds in Manifests:** Declare explicit minimum version bounds in files like `agents.md`, `go.mod`, `package.json`, or `pubspec.yaml`. The agent should be instructed (as we have done in our `agents.md`) to read and respect these minimum bounds.
