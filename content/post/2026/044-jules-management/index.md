...
This is different from a **Jules run completion**. A run-completion message may also be copied into the management-LLM conversation for review, but once the run has published or updated a GitHub branch or pull request, substantive continuation and correction belong in the GitHub review loop. The management LLM should inspect the actual remote state and, when more Jules implementation work is required, post a **new `@jules` comment on the relevant pull request**. It must not treat the copied completion prose as another joobq merely because both kinds of Jules output can arrive in the management conversation as pasted text.

The transport rule is therefore explicit: **joobq → human copy/paste → Jules app; run completion → GitHub state → management review → new `@jules` GitHub instruction when further implementation is needed.** A joobq can answer or unblock the currently paused Jules app interaction, but it is not a substitute control channel for resuming corrective work after a completed run has handed the work back through GitHub.
...
10. Treat `joobq` / `JOOBQ` as the acronym for **Jules out-of-band question**: a question copied from the Jules interface into the management-LLM conversation, outside the normal GitHub review loop. Return the answer as a copy/paste payload for the Jules interface; do not post it to GitHub or add `@jules` unless the human explicitly asks for GitHub delivery too. Do not classify ordinary Jules run-completion prose as joobq merely because the human copied it into chat: after a run has returned work through a GitHub branch or PR, inspect that durable state and send further implementation instructions through a new `@jules` GitHub comment. Interpret older `OOBJQ` wording as the same out-of-band event when encountered.
