...
Cross-repository issue creation has an additional guardrail. If the target repository has **more than 5 GitHub stars**, the management layer must never create a new issue there unless the human has explicitly approved or instructed that cross-repository issue creation. It may still search that repository, inspect existing issues, draft a candidate issue, and link or report relevant existing issues. This rule applies even when the discovered problem appears to belong upstream or in another project: popularity increases the chance that autonomous issue creation would become unsolicited interaction with a wider community.
...
7. Keep actionable discoveries durable, but do not create new issues autonomously. Search existing issues first, consolidate or enrich an appropriate existing issue when warranted, and present genuinely new candidate issues to the human for confirmation. Create them only after an explicit yes, then return their URLs. For cross-repository issue creation, apply a stricter guardrail: if the target repository has more than 5 GitHub stars, never create a new issue there without explicit human approval or instruction; searching, drafting, and linking existing issues remain allowed.
