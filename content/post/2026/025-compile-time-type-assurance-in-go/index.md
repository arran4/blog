...
3.  **`mappers.ReverseWords`**: This is the concrete implementation (in this case, seemingly a function or a value that should conform to the interface).

### Why Do This?

The magic happens during compilation. If `mappers.ReverseWords` does not implement the `strings2.WordMapper` interface—perhaps because a method signature changed or a method is missing—the Go compiler will throw an error immediately:

```text
cannot use mappers.ReverseWords (type ...) as type strings2.WordMapper in assignment:
	mappers.ReverseWords does not implement strings2.WordMapper (missing ... method)
```
